...
public class WxAccountCrawler extends BreadthCrawler {
    ...
    public void visit(Page page, CrawlDatums next) {
        ...
        if (page.matchType("account_search")) {
            ...
            //适应数据库中content大小
            //content = content.substring(71);
            //将页面key写入文件中用来去重
            writeHistoryKey(page.key());
            //持久化到数据库
            writeNewstoDB(page.key(),content,account);
    ...
    public synchronized void writeNewstoDB(String title, String content,String account) throws Exception {
...
//如果数据库中没有相关的表这里需要添加建表操作
        ...
        if (jdbcTemplate != null) {
            int updates=jdbcTemplate.update("insert into news"
                           +" (title,account, content, image ,like_count, comment_count, created_date, user_id) value(?,?,?,?,?,?,?,?)",
                    title,account,content, "http://images.nowcoder.com/head/23m.png", 0, 0, new Date(), 3);
            if(updates==1){
